"""IE-1 Interpretation module registry tests."""
...
def test_all_modules_registered_none_implemented() -> None:
    ...
    registry = InterpretationModuleRegistry.default()
...
def test_summary_depends_on_all_prior_modules() -> None:
    ...
    with pytest.raises(InterpretationFoundationError, match="missing_dependencies:summary"):
...
def test_registry_serialization_is_deterministic() -> None:
    ...
    first = InterpretationModuleRegistry.default().to_list()
    second = InterpretationModuleRegistry.default().to_list()
    assert first == second
