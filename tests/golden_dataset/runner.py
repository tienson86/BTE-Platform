...
from tests.golden_dataset.golden_adapter import (
    build_rule_context,
    is_birth_input,
    to_v1_report,
)
...
def run_case(
    ...
    try:
        if is_birth_input(chart):
            rule_context = build_rule_context(chart)
            interpretation = engine.run(rule_context)
            output_payload = to_v1_report(interpretation, rule_context)
        else:
            result = engine.interpret(chart)
            if is_dataclass(result):
                # V1 shape only — drop internal metadata keys for compare.
                raw = asdict(result)
                output_payload = {
