"""Runtime integration for Action Builder (N-IMP-08)."""

from __future__ import annotations

from typing import Any

from engines.narrative_v2.action import ActionPlanNarrative
...
def test_a19_runtime_build_action_returns_plan(
    ...
    result = runtime.pipeline.build_action()
    assert isinstance(result, StageResult)
    assert result.status == "implemented"
    assert isinstance(result.payload, ActionPlanNarrative)
    assert runtime.context is not None
    assert runtime.context.action is result.payload


def test_a20_commercial_remains_not_implemented(case_0001_canonical: dict[str, Any]) -> None:
    runtime = NarrativeRuntime()
    runtime.initialize(case_0001_canonical)
    runtime.pipeline.build_evidence()
    runtime.pipeline.build_reasoning()
...
def test_a21_shadow_mode_unchanged(case_0001_canonical: dict[str, Any]) -> None:
    runtime = NarrativeRuntime()
