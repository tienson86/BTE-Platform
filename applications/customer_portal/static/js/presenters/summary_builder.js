...
(function (global) {
  ...
  function formatLabel(raw) {
    ...
    if (ELEMENT_LABELS[raw] || ELEMENT_LABELS[String(raw)]) {
      return ELEMENT_LABELS[raw] || ELEMENT_LABELS[String(raw)];
    }
    var key = String(raw).trim().toLowerCase().replace(/\s+/g, "_");
    if (PATTERN_LABELS[key]) return PATTERN_LABELS[key];
    if (/^[a-z0-9_]+$/i.test(String(raw))) return MISSING;
    return text;
  }
  ...
  function buildBasic(data, input) {
    ...
    var solarDate = cal.solar_date
      ? present(cal.solar_date)
      : formatDmy(
  ...
  function extractNamedScores(source) {
    ...
    if (Array.isArray(source)) {
      source.forEach(function (item) {
        ...
        var label = item.label || item.name || item.element || item.god || null;
        var value = item.value != null ? item.value : item.score != null ? item.score : null;
        var mapped = label != null ? formatLabel(label) : MISSING;
        if (mapped !== MISSING && value != null) rows.push({ label: mapped, value: value });
      });
      return rows.length ? rows : null;
    }
    if (typeof source === "object") {
      Object.keys(source).forEach(function (key) {
        var mapped = formatLabel(key);
        if (mapped === MISSING) return;
        var val = source[key];
        if (typeof val === "number") rows.push({ label: mapped, value: val });
        else if (val && typeof val === "object" && val.score != null) {
          rows.push({ label: mapped, value: val.score });
        }
      });
      return rows.length ? rows : null;
    }
  ...
  function buildSeries(score, keys, detailsKey, scalarKey, fallbackLabel) {
    ...
    if (details) {
      var nested = extractNamedScores(details.details);
      if (nested) return nested;
      if (details.score != null) {
        return [{ label: fallbackLabel, value: details.score }];
      }
    }
  ...
  function buildShensha(bazi) {
    ...
    if (Array.isArray(raw)) {
      return raw
        ...
        .filter(function (v) {
          return v !== MISSING && !/^[a-z0-9_]+$/i.test(v);
        });
    }
    if (typeof raw === "object") {
      return Object.keys(raw)
        .map(function (k) {
          var v = raw[k];
          if (v === true) {
            var mapped = formatLabel(k);
            return mapped === MISSING ? null : mapped;
          }
          if (v == null || v === false || v === "") return null;
          var valueText = present(v);
          return valueText === MISSING ? null : valueText;
        })
        .filter(Boolean);
    }
    var one = present(raw);
  ...
  function buildHighlight(dayMaster, overview, score) {
    return {
      ...
      total_score: present(pick(score || {}, ["total_score"])),
      grade: present(pick(score || {}, ["grade"])),
    };
  }
