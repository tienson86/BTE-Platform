...
(function (global) {
  ...
  /**
   * Returns true for lines that look like raw unaccented Vietnamese rule text
   * e.g. "Kiep Tai Cach", "Khi mua hien tai sinh tro..."
   * These are Engine internal strings that should not be displayed.
   */
  // Lines that are raw unaccented rule codes or internal key lines
  var RAW_RULE_LINE = /^[A-Za-z][A-Za-z0-9 _\-/()]{4,}$/;
  var INTERNAL_CODE_LINE = /\b(?:FPR|SPR|PAT|PSC|PPR|SER|SDR|CBR|PC)\d+\b|\bstatus\s*=/i;

  function filterMarkdownLines(md) {
    if (!md) return md;
    ...
    while (i < lines.length) {
      ...
      // Always keep empty lines and headings
      if (!trimmed || /^#{1,6}\s/.test(trimmed)) {
        result.push(line);
        i++;
        continue;
      }
      // Drop internal code lines
      if (INTERNAL_CODE_LINE.test(trimmed)) { i++; continue; }
      // Drop raw unaccented text (likely rule slugs from Engine)
      if (RAW_RULE_LINE.test(trimmed)) { i++; continue; }
      result.push(line);
      i++;
    }
    // Remove headings that have no content after them (only empty lines before next heading or EOF)
    var filtered = result;
    var clean = [];
    var j = 0;
    while (j < filtered.length) {
      var fline = filtered[j];
      if (/^#{2,6}\s/.test(fline.trim())) {
        // Check if there is any non-empty non-heading line before next heading
        var hasContent = false;
        var k = j + 1;
        while (k < filtered.length && !/^#{1,6}\s/.test(filtered[k].trim())) {
          if (filtered[k].trim()) { hasContent = true; break; }
          k++;
        }
        if (hasContent) clean.push(fline);
        j++;
        continue;
      }
      clean.push(fline);
  ...
  function renderNarrative(narrative) {
    try {
      ...
      if (source.kind === "markdown") {
        var filteredContent = filterMarkdownLines(source.content);
        tocHtml = buildToc(extractMarkdownHeadings(filteredContent));
        bodyHtml = renderMarkdownDocument(filteredContent);
        copySource = plainTextFromMarkdown(filteredContent);
