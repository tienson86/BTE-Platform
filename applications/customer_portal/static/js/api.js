(function (global) {
  ...
  function resultStore() {
    ...
    if (!store) throw new Error("BtePortal.ResultStore is not loaded");
    return store;
  }

  function saveLastResult(payload) {
    return resultStore().save(payload);
  }

  function getLastResult() {
    return resultStore().load();
  }

  function getHistory() {
    return resultStore().loadHistory();
  }
  ...
  // Merge so the ResultStore registered by result_store.js survives.
  global.BtePortal = Object.assign(global.BtePortal || {}, {
    getToken,
    setSession,
    clearSession,
    getUser,
