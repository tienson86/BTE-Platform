"""UI-06 Card 03 Five Elements — grid freeze, copy-only adapter, routing."""
...
_ADAPTER = _SRC / "screens" / "commercial_dashboard" / "fiveElementsAdapter.ts"
_CARD = _SRC / "screens" / "commercial_dashboard" / "FiveElementsCard.tsx"
...
def test_f2_f18_grid_spans_unchanged() -> None:
    ...
    assert 'card.id === "bazi"' in grid
    assert "SkeletonCard" in grid


def test_f14_five_elements_does_not_calculate_astrology() -> None:
    ...
    for source in (adapter, card):
        ...
        assert "wuxing_score" not in source
        assert "Thiếu Thủy" not in source
        assert "Bổ Thủy" not in source


def test_f20_result_store_and_routing_remain() -> None:
