"""UI-05 Card 02 BaZi Structure — grid freeze, copy-only adapter, routing."""
...
_ADAPTER = _SRC / "screens" / "commercial_dashboard" / "baziAdapter.ts"
_CARD = _SRC / "screens" / "commercial_dashboard" / "BaziCard.tsx"
...
def test_b2_b17_grid_spans_unchanged() -> None:
...
def test_b12_bazi_does_not_calculate_astrology() -> None:
    ...
    for source in (adapter, card):
        ...
        assert "calculateHidden" not in source
        assert "inferTenGod" not in source
    assert "BÁT TỰ" in card or "model.title" in card


def test_b20_result_store_and_routing_remain() -> None:
