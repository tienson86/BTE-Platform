...
describe("G1-03 canonical Pattern binding", () => {
  it("Canonical Desktop Cách cục is Chính Ấn with Căn cứ evidence", () => {
    ...
    const canCu = vm.s01.conditions.rows.find((row) => row.label === "Căn cứ");
    expect(canCu?.value).toContain("Nguyệt lệnh Sửu");
    expect(canCu?.value).toContain("khí chính Kỷ");
    expect(canCu?.value).not.toContain("thành cách");
    expect(canCu?.value).not.toContain("Chính Quan");
  });

  it("Full Report Cách cục and Căn cứ use canonical Pattern", () => {
