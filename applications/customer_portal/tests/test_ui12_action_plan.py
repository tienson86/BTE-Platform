"""UI-12 Card 09 Action Plan — grid freeze, copy-only adapter, routing."""
...
_ADAPTER = _SRC / "screens" / "commercial_dashboard" / "actionPlanAdapter.ts"
_CARD = _SRC / "screens" / "commercial_dashboard" / "ActionPlanCard.tsx"
...
def test_a2_a17_grid_spans_unchanged() -> None:
...
def test_a5_a8_adapter_does_not_reason() -> None:
    ...
    for source in (adapter, card):
        ...
        assert "màu đỏ" not in source
        assert "hướng Nam" not in source
        assert "kết hôn" not in source


def test_a20_result_store_and_routing_remain() -> None:
