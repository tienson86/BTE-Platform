"""RB19 Number Energy public runtime visual freeze. Does not intercept the public bundle."""
...
RESULT_ROUTE = f"{BASE}/result"
DESKTOP = {"width": 1440, "height": 900}
MOBILE = {"width": 390, "height": 844}
VEHICLE_MESSAGE = "Phân tích biển số chưa được hỗ trợ lúc này."
FORBIDDEN = [
    "verified_by_runtime",
    "fixture_id",
    ...
    return page.evaluate(
        """() => {
          ...
          return {
            ...
            ctaVisible: ctaRect
              ? ctaRect.right <= window.innerWidth + 1 && ctaRect.left >= -1 && ctaRect.width > 40
              : null,
...
def _submit_phone(page, number: str, *, wait_result: bool = True) -> None:
    page.locator("[data-testid='number-input']").fill(number)
...
def _submit_vehicle(page) -> None:
    page.locator("[data-testid='analysis-type-car_plate']").click()
    page.locator("[data-testid='number-input']").fill("30A-123.45")
    page.locator("[data-testid='analysis-submit']").click()
    page.locator("[data-testid='input-error']").wait_for(timeout=8000)
...
def _unexpected_overflow(block: dict) -> dict:
    ...
    return {
        ...
        "ctaVisible": block.get("ctaVisible"),
...
def main() -> None:
    ...
    try:
        ...
        with sync_playwright() as playwright:
            ...
            page.screenshot(path=str(OUT / "01_public_input_desktop.png"), full_page=True)
            report["inputDesktop"] = {
                ...
                "staticSampleNote": page.locator("[data-testid='static-sample-note']").count(),
                "analyzeBeforeSubmit": analyze_before_submit,
                "text": _visible_text(page),
                "overflow": _overflow_report(page),
            }

            _submit_phone(page, "0328278786")
            page.locator("[data-testid='result-hero']").scroll_into_view_if_needed()
            page.screenshot(path=str(OUT / "02_public_result_top_desktop.png"), full_page=False)
            page.screenshot(path=str(OUT / "03_public_result_full_desktop.png"), full_page=True)
            report["successDesktop"] = {
                ...
                "overflow": _overflow_report(page),
                "text": _visible_text(page),
                "analyzeCalls": _analyze_calls(requests),
                "triples": page.locator("[data-testid^='triple-card-']").count(),
                "triple787": page.locator("[data-testid='triple-digits-4']").inner_text(),
                "triple878": page.locator("[data-testid='triple-digits-5']").inner_text(),
                "scoreText": page.locator("[data-testid='score-breakdown']").inner_text(),
            }

            page.set_viewport_size(MOBILE)
            _goto_number_energy(page)
            page.screenshot(path=str(OUT / "06_public_input_mobile.png"), full_page=True)
            _submit_phone(page, "0328278786")
            page.locator("[data-testid='result-hero']").scroll_into_view_if_needed()
            page.screenshot(path=str(OUT / "07_public_result_top_mobile.png"), full_page=False)
            page.screenshot(path=str(OUT / "08_public_result_full_mobile.png"), full_page=True)
            ...
            error_requests = _track(error_page)
            _fail_analyze(error_page)
            _goto_number_energy(error_page)
            _submit_phone(error_page, "0328278786", wait_result=False)
            ...
            error_text = _visible_text(error_page)
            error_page.evaluate("window.scrollTo(0, 0)")
            error_page.screenshot(path=str(OUT / "04_public_error_desktop.png"), full_page=False)
            error_page.set_viewport_size(MOBILE)
            _goto_number_energy(error_page)
            _submit_phone(error_page, "0328278786", wait_result=False)
            error_page.locator("[data-testid='input-error']").wait_for(timeout=15000)
            error_page.evaluate("window.scrollTo(0, 0)")
            error_page.screenshot(path=str(OUT / "09_public_error_mobile.png"), full_page=True)
            report["error"] = {
            ...
            vehicle_page = browser.new_page(viewport=DESKTOP)
            vehicle_requests = _track(vehicle_page)
            _goto_number_energy(vehicle_page)
            _submit_vehicle(vehicle_page)
            vehicle_hidden = vehicle_page.locator("[data-testid='result-section']").evaluate("el => el.hidden")
            vehicle_page.evaluate("window.scrollTo(0, 0)")
            vehicle_page.screenshot(path=str(OUT / "05_public_vehicle_blocked_desktop.png"), full_page=False)
            vehicle_desktop_message = vehicle_page.locator("[data-testid='input-error']").inner_text()
            vehicle_page.set_viewport_size(MOBILE)
            _goto_number_energy(vehicle_page)
            _submit_vehicle(vehicle_page)
            vehicle_page.evaluate("window.scrollTo(0, 0)")
            vehicle_page.screenshot(path=str(OUT / "10_public_vehicle_blocked_mobile.png"), full_page=True)
            report["vehicle"] = {
                "resultHidden": vehicle_hidden,
                "message": vehicle_desktop_message,
                "mobileMessage": vehicle_page.locator("[data-testid='input-error']").inner_text(),
                "analyzeCalls": _analyze_calls(vehicle_requests),
                "text": _visible_text(vehicle_page),
            }
            vehicle_page.close()

            invalid_page = browser.new_page(viewport=DESKTOP)
            invalid_requests = _track(invalid_page)
            _goto_number_energy(invalid_page)
            invalid_page.locator("[data-testid='number-input']").fill("abcd")
            invalid_page.locator("[data-testid='analysis-submit']").click()
            invalid_page.locator("[data-testid='input-error']").wait_for(timeout=8000)
            report["invalidPhone"] = {
                "analyzeCalls": _analyze_calls(invalid_requests),
                "message": invalid_page.locator("[data-testid='input-error']").inner_text(),
                "resultHidden": invalid_page.locator("[data-testid='result-section']").evaluate(
                    "el => el.hidden"
                ),
            }
            invalid_page.close()

            other_page = browser.new_page(viewport=DESKTOP)
            other_requests = _track(other_page)
            _goto_number_energy(other_page)
            other_page.locator("[data-testid='number-input']").fill("0868271327")
            other_page.locator("[data-testid='analysis-submit']").click()
            deadline = time.time() + 20
            while time.time() < deadline and not _analyze_calls(other_requests):
                other_page.wait_for_timeout(200)
            format_error = False
            if other_page.locator("[data-testid='input-error']").count():
                format_error = "định dạng" in other_page.locator("[data-testid='input-error']").inner_text()
            report["otherPhone"] = {
                "analyzeCalls": _analyze_calls(other_requests),
                "resultHidden": other_page.locator("[data-testid='result-section']").evaluate(
                    "el => el.hidden"
                ),
                "hasFormatError": format_error,
            }
            other_page.close()

            result_page = browser.new_page(viewport=DESKTOP)
            result_page.goto(RESULT_ROUTE, wait_until="networkidle")
            report["resultNav"] = _nav_state(result_page)
            result_page.close()
            browser.close()
    finally:
    ...
    for label, key in (("error", "text"), ("vehicle", "text")):
        blob = report[label].get(key) or ""
        for token in _token_leaks(blob):
            leaks.append({"surface": label, "token": token})
        if any(token in blob for token in ("Traceback", "energy_id", "HTTP_ERROR")):
            leaks.append({"surface": label, "token": "technical-error"})
    ...
    summary = {
        ...
        "successDesktop": {
            ...
            "triples": report["successDesktop"]["triples"],
            "triple787": report["successDesktop"]["triple787"],
            "triple878": report["successDesktop"]["triple878"],
            "scoreHasPercent": "%" in report["successDesktop"]["scoreText"],
        },
        "successMobile": {
            ...
            "triples": report["successMobile"]["triples"],
            "triple787": report["successMobile"]["triple787"],
            "triple878": report["successMobile"]["triple878"],
            "scoreHasPercent": "%" in report["successMobile"]["scoreText"],
        },
        "error": {
            "resultHidden": report["error"]["resultHidden"],
            "message": report["error"]["message"],
            "analyzeCalls": len(report["error"]["analyzeCalls"]),
        },
        "vehicle": {
            "resultHidden": report["vehicle"]["resultHidden"],
            "message": report["vehicle"]["message"],
            "mobileMessage": report["vehicle"]["mobileMessage"],
            "analyzeCalls": len(report["vehicle"]["analyzeCalls"]),
        },
        "invalidPhone": {
            "analyzeCalls": len(report["invalidPhone"]["analyzeCalls"]),
            "resultHidden": report["invalidPhone"]["resultHidden"],
            "message": report["invalidPhone"]["message"],
        },
        "otherPhone": {
            "analyzeCalls": len(report["otherPhone"]["analyzeCalls"]),
            "hasFormatError": report["otherPhone"]["hasFormatError"],
        },
        "resultNav": report["resultNav"],
        "screenshots": [
        ...
        json.dumps(
            {
                ...
                "errorHidden": summary["error"]["resultHidden"],
                "vehicleCalls": summary["vehicle"]["analyzeCalls"],
                "otherPhoneCalls": summary["otherPhone"]["analyzeCalls"],
                "resultNavActive": summary["resultNav"]["navActive"],
            },
            ensure_ascii=True,
            indent=2,
        )
