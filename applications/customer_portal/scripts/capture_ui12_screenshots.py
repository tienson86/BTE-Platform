"""Capture UI-12 Action Plan Phase A fixture and Phase B live screenshots."""
...
OUT = REPO / "knowledge" / "commercial_dashboard" / "implementation" / "ui12"
API_PORT = 8000
PORTAL_PORT = 8081
...
def _expand(page) -> None:
    toggle = page.locator('[data-card="action-plan"] button.bte-ap__toggle')
...
def _assert_safe(text: str) -> None:
    ...
    if any(token in lowered for token in forbidden):
        raise RuntimeError("Action Plan Card leaked technical or English product labels")
...
def main() -> None:
    ...
    try:
        ...
        with sync_playwright() as playwright:
            ...
            page.wait_for_selector('[data-card="action-plan"][data-implemented="action-plan"]')
            _collapse(page)
            _shot_element(page, '[data-card="action-plan"]', OUT / "01_action_plan_visual_desktop.png")
            _expand(page)
            _shot_element(page, '[data-card="action-plan"]', OUT / "02_action_plan_visual_expanded.png")
            page.set_viewport_size({"width": 390, "height": 1100})
            page.wait_for_timeout(400)
            _collapse(page)
            _shot_element(page, '[data-card="action-plan"]', OUT / "03_action_plan_visual_mobile.png")
            ...
            page.wait_for_selector('[data-card="action-plan"][data-implemented="action-plan"]')
            if page.locator("[data-card][data-skeleton='true']").count():
                raise RuntimeError("Dashboard still has skeleton cards")
            _collapse(page)
            live = page.locator('[data-card="action-plan"]').inner_text()
            _assert_safe(live)
            if "Chưa có đủ dữ liệu để tạo kế hoạch hành động" in live:
                raise RuntimeError("Live CASE-0001 Action Plan is empty")
            _shot_element(page, '[data-card="action-plan"]', OUT / "04_action_plan_live_case0001.png")
            _expand(page)
            _shot_element(page, '[data-card="action-plan"]', OUT / "05_action_plan_live_expanded_case0001.png")
            page.set_viewport_size({"width": 390, "height": 1400})
            page.wait_for_timeout(400)
            _collapse(page)
            box = page.evaluate("() => ({ sw: document.documentElement.scrollWidth, iw: window.innerWidth })")
            if box["sw"] > box["iw"] + 1:
                raise RuntimeError("Mobile page overflows horizontally")
            _shot_element(page, '[data-card="action-plan"]', OUT / "06_action_plan_live_mobile.png")
            page.set_viewport_size({"width": 1440, "height": 2800})
            page.wait_for_timeout(400)
            _collapse(page)
            page.locator(".bte-cdash").first.screenshot(path=str(OUT / "07_full_dashboard_ui12.png"))
            browser.close()
    finally:
