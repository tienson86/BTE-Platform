"""P7-IMP-08 live proof: Evidence Priority, /result, diagnostics."""
...
def main() -> None:
    """Restart runtime and capture Evidence Priority live proof."""
    ...
    try:
        ...
        evidence = analyze_data.get("evidence_priority") if isinstance(analyze_data.get("evidence_priority"), dict) else {}
        ...
        overview = SHOTS / "p7_imp_08_result_overview.png"
        mingju = SHOTS / "p7_imp_08_mingju.png"
        evidence_shot = SHOTS / "p7_imp_08_evidence_priority.png"
        ecosystem_shot = SHOTS / "p7_imp_08_ten_gods_ecosystem.png"
        shen_eco_shot = SHOTS / "p7_imp_08_shen_sha_ecosystem.png"
        mobile = SHOTS / "p7_imp_08_mobile_evidence_priority.png"
        diag_shot = SHOTS / "p7_imp_08_diagnostics.png"
        with sync_playwright() as playwright:
            ...
            if "TR-P7-" in content or "E-DI-EPR-" in content or "mingju_result_id" in content:
                raise RuntimeError("Pack 07 debug IDs leaked onto customer /result")
            page.screenshot(path=str(overview), full_page=False)
            focus = page.locator("[data-overview-section='evidence-priority']")
            focus.scroll_into_view_if_needed()
            focus.screenshot(path=str(evidence_shot))
            ...
            page.locator("[data-tg-section='ecosystem']").scroll_into_view_if_needed()
            ...
            page.locator("[data-ss-section='ecosystem']").scroll_into_view_if_needed()
            ...
            mobile_focus = mobile_page.locator("[data-overview-section='evidence-priority']")
        ...
        if "E-DI-EPR-" in dump or "TR-P7-EPR-" in dump:
            raise RuntimeError("Evidence Priority traces leaked onto public analyze payload")
        if diag_body.get("evidence_priority") != "PASS":
            raise RuntimeError(f"Evidence Priority diagnostics not PASS: {diag_body.get('evidence_priority')}")
        proof = {
            ...
            "screenshots": {
                ...
                "evidence_priority": str(evidence_shot.relative_to(REPO)).replace("\\", "/"),
                ...
                "mobile_evidence_priority": str(mobile.relative_to(REPO)).replace("\\", "/"),
                "diagnostics": str(diag_shot.relative_to(REPO)).replace("\\", "/"),
            },
        }
        (OUT / "P7-IMP-08_diagnostics.json").write_text(
