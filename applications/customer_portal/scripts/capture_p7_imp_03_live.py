"""P7-IMP-03 live proof: unchanged /result + Pack 07 dev diagnostics artifact."""
...
def main() -> None:
    """Restart runtime, prove customer UI, capture screenshots and diagnostics JSON."""
    ...
    try:
        ...
        diag_status, diag = _post("/api/v1/dev/pack07/diagnostics", CASE) if False else _get(
        ...
        result_code = urllib.request.urlopen(f"{BASE}/result", timeout=20).status
        proof = {
            ...
            "result_portal": result_code,
            "diagnostics_get": {"status": diag_status, "body": diag},
        ...
        desktop = SHOTS / "p7_imp_03_result_desktop.png"
        mobile = SHOTS / "p7_imp_03_result_mobile.png"
        with sync_playwright() as playwright:
            ...
            if "pack07" in (page.content() or "").lower() and "NOT_IMPLEMENTED" in (page.content() or ""):
                raise RuntimeError("Pack 07 diagnostics leaked onto customer /result")
            ...
            mobile_page.goto(f"{BASE}/analyze", wait_until="networkidle")
            mobile_page.wait_for_selector("#analyzeForm")
            _fill(mobile_page)
            mobile_page.click("#btnAnalyze")
            mobile_page.wait_for_url("**/result", timeout=120000)
            mobile_page.wait_for_selector("[data-dashboard='commercial-v1']")
            mobile_page.screenshot(path=str(mobile), full_page=False)
            browser.close()
        proof["screenshots"] = {
            "desktop": str(desktop.relative_to(REPO)).replace("\\", "/"),
            "mobile": str(mobile.relative_to(REPO)).replace("\\", "/"),
        }
        (OUT / "P7-IMP-03_diagnostics.json").write_text(
            json.dumps(proof, ensure_ascii=False, indent=2),
            encoding="utf-8",
        )
        print(json.dumps(proof, ensure_ascii=False, indent=2)[:4000])
