"""Capture UI-09 ShenSha visual fixture and live CASE-0001 screenshots."""
...
OUT = REPO / "knowledge" / "commercial_dashboard" / "implementation" / "ui09"
API_PORT = 8000
PORTAL_PORT = 8081
...
def _expand(page) -> None:
    toggle = page.locator('[data-card="shensha"] button.bte-ss__toggle')
...
def _report_live_contract(payload: dict[str, object]) -> None:
    ...
    matches = bazi.get("shensha_matches") if isinstance(bazi.get("shensha_matches"), list) else []
    names = bazi.get("shensha") if isinstance(bazi.get("shensha"), list) else []
    sample = matches[0] if matches and isinstance(matches[0], dict) else {}
    snapshot = {
        ...
        "has_category": any(isinstance(item, dict) and "category" in item for item in matches),
        "has_meaning": any(isinstance(item, dict) and "meaning" in item for item in matches),
        "has_summary": "summary" in bazi,
        "has_occurrences": any(
            isinstance(item, dict) and bool(item.get("occurrences")) for item in matches
        ),
    }
    ...
    print("UI-09 live shensha count:", snapshot["match_count"])


def main() -> None:
    """Capture UI-09 ShenSha fixture and live screenshots."""
    ...
    try:
        ...
        with sync_playwright() as playwright:
            ...
            page.wait_for_selector('[data-card="shensha"][data-implemented="shensha"]')
            _collapse(page)
            _shot_element(page, '[data-card="shensha"]', OUT / "01_shensha_visual_desktop.png")
            _expand(page)
            _shot_element(page, '[data-card="shensha"]', OUT / "02_shensha_visual_expanded.png")
            _collapse(page)
            _shot_row03(page, OUT / "03_row03_visual_shensha_luck.png")

            page.set_viewport_size({"width": 390, "height": 844})
            page.wait_for_timeout(400)
            _collapse(page)
            _shot_element(page, '[data-card="shensha"]', OUT / "04_shensha_visual_mobile.png")
            ...
            _collapse(page)
            _shot_element(page, '[data-card="shensha"]', OUT / "05_shensha_live_case0001.png")
            _expand(page)
            _shot_element(page, '[data-card="shensha"]', OUT / "06_shensha_live_expanded_case0001.png")
            _collapse(page)
            _shot_row03(page, OUT / "07_row03_live_case0001.png")
            page.screenshot(path=str(OUT / "09_full_dashboard_ui09.png"), full_page=True)

            page.set_viewport_size({"width": 390, "height": 844})
            page.wait_for_timeout(400)
            _collapse(page)
            _shot_element(page, '[data-card="shensha"]', OUT / "08_shensha_live_mobile.png")
