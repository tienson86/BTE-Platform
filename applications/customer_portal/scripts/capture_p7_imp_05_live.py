"""P7-IMP-05 live proof: combinations, ecosystem, /result THẬP THẦN, diagnostics."""
...
def main() -> None:
    """Restart runtime and capture combinations/ecosystem live proof."""
    ...
    try:
        ...
        overview = SHOTS / "p7_imp_05_result_overview.png"
        individuals = SHOTS / "p7_imp_05_ten_gods_desktop.png"
        relations_shot = SHOTS / "p7_imp_05_combinations.png"
        ecosystem_shot = SHOTS / "p7_imp_05_ecosystem.png"
        expanded = SHOTS / "p7_imp_05_combination_expanded.png"
        mobile = SHOTS / "p7_imp_05_ten_gods_mobile.png"
        with sync_playwright() as playwright:
            ...
            if "TR-P7-" in content or "shi_shen_generates_wealth" in content:
                raise RuntimeError("Pack 07 debug IDs leaked onto customer /result")
            page.screenshot(path=str(overview), full_page=False)
            card = page.locator("[data-card='ten-gods']")
            card.scroll_into_view_if_needed()
            page.wait_for_selector("[data-tg-section='detailed']")
            card.screenshot(path=str(individuals))
            page.wait_for_selector("[data-tg-section='relations']")
            page.locator("[data-tg-section='relations']").screenshot(path=str(relations_shot))
            ...
            relation_toggle = page.locator("[data-tg-section='relations'] .bte-tg__detail-toggle")
            if relation_toggle.count():
                relation_toggle.first.click()
                page.wait_for_timeout(400)
            page.locator("[data-tg-section='relations']").screenshot(path=str(expanded))
        ...
        if not isinstance(detailed, dict) or not detailed.get("items"):
            raise RuntimeError("Analyze payload missing Ten Gods detailed projection")
        if not isinstance(relations, dict) or "items" not in relations:
            raise RuntimeError("Analyze payload missing Ten God relations projection")
        if not isinstance(ecosystem, dict) or "driver" not in ecosystem:
            raise RuntimeError("Analyze payload missing Ten Gods ecosystem projection")
        proof = {
            ...
            "analyze": {
                ...
                "detailed_count": len(detailed.get("items") or []),
                "relations_count": len(relations.get("items") or []),
                "relations_names": [item.get("name") for item in relations.get("items") or []],
                "ecosystem": ecosystem,
            },
            ...
            "screenshots": {
                ...
                "ten_gods_desktop": str(individuals.relative_to(REPO)).replace("\\", "/"),
                "combinations": str(relations_shot.relative_to(REPO)).replace("\\", "/"),
                "ecosystem": str(ecosystem_shot.relative_to(REPO)).replace("\\", "/"),
                "combination_expanded": str(expanded.relative_to(REPO)).replace("\\", "/"),
                "ten_gods_mobile": str(mobile.relative_to(REPO)).replace("\\", "/"),
            },
        }
        (OUT / "P7-IMP-05_diagnostics.json").write_text(
            json.dumps(proof, ensure_ascii=False, indent=2),
            encoding="utf-8",
        )
        print(json.dumps(proof, ensure_ascii=False, indent=2)[:6000])
