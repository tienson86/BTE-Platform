"""Capture DS-08 commercial visual freeze screenshots."""
    ...
    try:
        ...
        with sync_playwright() as playwright:
            ...
            desktop.locator("[data-testid='search-row']").screenshot(
                path=str(OUT / "ds08_01_input_person.png")
            )
            desktop.locator("#dsResults").screenshot(path=str(OUT / "ds08_02_desktop.png"))
            desktop.locator("[data-testid='ranked-card']").first.screenshot(
                path=str(OUT / "ds08_03_result_card.png")
            )
            tablet = browser.new_page(viewport={"width": 768, "height": 1400})
            _search(tablet)
            tablet.screenshot(path=str(OUT / "ds08_04_tablet.png"), full_page=True)
            phone = browser.new_page(viewport={"width": 375, "height": 1100})
            _search(phone)
            phone.screenshot(path=str(OUT / "ds08_05_phone.png"), full_page=True)
            browser.close()
    finally:
        api.terminate()
        portal.terminate()
