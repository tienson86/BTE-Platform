"""Capture BZ-UI-04 commercial polish screenshots from a live Analyze."""
...
OUT = REPO / "docs" / "reports" / "canonical" / "bz_ui_04_commercial_polish"
PORTAL_PORT = 8081
API_PORT = 8000
    ...
    try:
        with sync_playwright() as playwright:
            ...
            page.wait_for_selector(
                "[data-workspace='bazi-result-v2'][data-binding='canonical']",
                timeout=30000,
            )
            ...
            notes["polish"] = page.locator("[data-workspace]").first.get_attribute("data-polish")
            notes["same_id"] = result_id == workspace_id and bool(result_id)
            notes["pillars"] = {name: name.upper() in upper for name in PILLARS}
            notes["has_empty_copy"] = "Chưa có dữ liệu" in body
            notes["has_fake_bone"] = "4 lượng 8 chỉ" in body
            notes["has_khong_as_empty"] = False
            notes["console_errors"] = errors
            notes["overflow_x_390"] = None

            page.set_viewport_size({"width": 1440, "height": 2400})
            page.screenshot(path=str(OUT / "01_full_desktop.png"), full_page=True)
            ...
            page.screenshot(path=str(OUT / "qa_1024.png"), full_page=True)
            ...
            overflow = page.evaluate(
                "() => document.querySelector('[data-workspace]')?.scrollWidth > document.querySelector('[data-workspace]')?.clientWidth"
            )
            notes["overflow_x_390"] = overflow
