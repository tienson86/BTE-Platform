"""MC01-IMP-01 live proof: CASE-0001 MingJu runtime, /result, diagnostics."""
...
OUT = REPO / "implementation" / "pack_06"
...
def _diagnostics_html(data: dict[str, object]) -> str:
    ...
    return (
        ...
        "<h1>MC-01 / Pack 07 diagnostics</h1>"
...
def main() -> None:
    """Restart runtime and capture MC-01 live proof."""
    ...
    try:
        ...
        overview = SHOTS / "mc01_imp_01_result_overview.png"
        mingju = SHOTS / "mc01_imp_01_mingju.png"
        grade = SHOTS / "mc01_imp_01_grade_integrity.png"
        ten_gods_shot = SHOTS / "mc01_imp_01_ten_gods.png"
        ecosystem_shot = SHOTS / "mc01_imp_01_ten_gods_ecosystem.png"
        shen_shot = SHOTS / "mc01_imp_01_shen_sha.png"
        shen_eco_shot = SHOTS / "mc01_imp_01_shen_sha_ecosystem.png"
        diag_shot = SHOTS / "mc01_imp_01_diagnostics.png"
        with sync_playwright() as playwright:
            ...
            if "TR-P7-" in content or "mingju_result_id" in content or "content_hash" in content:
                raise RuntimeError("debug IDs leaked onto customer /result")
            ...
            grade_node = page.locator("[data-pat-grade], [data-pat-integrity]").first
            if grade_node.count():
                grade_node.scroll_into_view_if_needed()
                page.locator("[data-card='pattern']").screenshot(path=str(grade))
            else:
                pattern_card.screenshot(path=str(grade))
        ...
        if diag_body.get("mc01_reference") != "PASS":
            raise RuntimeError(f"MC-01 diagnostics not PASS: {diag_body.get('mc01_reference')}")
        proof = {
            ...
            "analyze": {
                ...
                "has_mingju": "mingju" in analyze_data,
                "pattern": {
                    "cach_cuc": pattern.get("cach_cuc"),
                    "structural_purity": pattern.get("structural_purity"),
                    "structural_strength": pattern.get("structural_strength"),
                    "structural_integrity": pattern.get("structural_integrity"),
                    "structural_grade": pattern.get("structural_grade"),
                    "customer_summary": pattern.get("customer_summary"),
                },
                "score_grade": score.get("grade"),
                "damage_ids": analyze_data.get("damage_ids"),
                "rescue_ids": analyze_data.get("rescue_ids"),
                "integrity": analyze_data.get("integrity"),
                "achievement": analyze_data.get("achievement"),
                "wealth_profile": analyze_data.get("wealth_profile"),
                "career_profile": analyze_data.get("career_profile"),
                ...
                "relations_states": [
                    item.get("state") for item in (relations.get("items") or []) if isinstance(item, dict)
                ]
                if isinstance(relations, dict)
                else [],
                "ecosystem": ecosystem,
            ...
            "screenshots": {
                ...
                "grade_integrity": str(grade.relative_to(REPO)).replace("\\", "/"),
        ...
        (OUT / "MC01-IMP-01_live_proof.json").write_text(
