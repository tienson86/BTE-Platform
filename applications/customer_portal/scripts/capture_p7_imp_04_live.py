"""P7-IMP-04 live proof: /result Thập Thần + Pack 07 diagnostics."""
...
def main() -> None:
    """Restart runtime, capture /result Thập Thần and Pack 07 diagnostics."""
    ...
    try:
        ...
        overview = SHOTS / "p7_imp_04_result_overview.png"
        desktop = SHOTS / "p7_imp_04_ten_gods_desktop.png"
        expanded = SHOTS / "p7_imp_04_ten_gods_expanded.png"
        mobile = SHOTS / "p7_imp_04_ten_gods_mobile.png"
        with sync_playwright() as playwright:
            browser = playwright.chromium.launch()
            desk = browser.new_context(viewport={"width": 1440, "height": 1100})
            page = desk.new_page()
            _open_result(page)
            if "TR-P7-TG-" in (page.content() or ""):
                raise RuntimeError("Ten God trace IDs leaked onto customer /result")
            ...
            page.locator("[data-tg-section='detailed'] .bte-tg__detail-toggle").first.click()
            page.wait_for_timeout(400)
            card.screenshot(path=str(expanded))
            ...
            toggle = mobile_page.locator("[data-card='ten-gods'] [data-mobile-toggle], [data-card='ten-gods'] .bte-mobile-toggle")
            if toggle.count():
                toggle.first.click()
        ...
        proof = {
            ...
            "analyze": {
                ...
                "detailed_state": detailed.get("state"),
            },
            ...
            "screenshots": {
                ...
                "ten_gods_desktop": str(desktop.relative_to(REPO)).replace("\\", "/"),
                "ten_gods_expanded": str(expanded.relative_to(REPO)).replace("\\", "/"),
                "ten_gods_mobile": str(mobile.relative_to(REPO)).replace("\\", "/"),
            },
        }
        (OUT / "P7-IMP-04_diagnostics.json").write_text(
            json.dumps(proof, ensure_ascii=False, indent=2),
            encoding="utf-8",
        )
        print(json.dumps(proof, ensure_ascii=False, indent=2)[:5000])
