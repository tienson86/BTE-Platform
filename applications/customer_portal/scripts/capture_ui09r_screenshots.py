"""Capture UI-09R ShenSha customer content screenshots."""
...
OUT = REPO / "knowledge" / "commercial_dashboard" / "implementation" / "ui09r"
BEFORE = (
    REPO
    / "knowledge"
...
<body style="margin:0;background:#f4f1ea;font-family:Segoe UI,sans-serif;color:#1f1b16;">
  <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;padding:16px;">
    <figure style="margin:0;background:#fff;border:1px solid #d7cfc4;border-radius:8px;padding:12px;">
      <figcaption style="font-weight:700;margin-bottom:8px;">BEFORE — chips only</figcaption>
      <img src="data:image/png;base64,__BEFORE__" style="width:100%;height:auto;" />
    </figure>
    <figure style="margin:0;background:#fff;border:1px solid #d7cfc4;border-radius:8px;padding:12px;">
      <figcaption style="font-weight:700;margin-bottom:8px;">AFTER — name, placement, meaning</figcaption>
...
def main() -> None:
    ...
    try:
        ...
        with sync_playwright() as playwright:
            ...
            text = page.locator('[data-card="shensha"]').inner_text()
            if "Tháng\nTháng" in text or "Xem chi tiết" in text:
                raise RuntimeError("ShenSha still shows orphan placements or empty expand")
            _shot_element(page, '[data-card="shensha"]', OUT / "02_shensha_customer_default.png")
            _shot_element(page, '[data-card="shensha"] .bte-ss__list', OUT / "03_shensha_customer_detail.png")
            _shot_row03(page, OUT / "04_row03_shensha_luck.png")
            page.set_viewport_size({"width": 390, "height": 900})
            page.wait_for_timeout(400)
            _shot_element(page, '[data-card="shensha"]', OUT / "05_shensha_mobile.png")
            page.set_viewport_size({"width": 1280, "height": 1100})
            _compose_before_after(page, OUT / "02_shensha_customer_default.png", OUT / "01_shensha_before_after.png")
            browser.close()
    finally:
