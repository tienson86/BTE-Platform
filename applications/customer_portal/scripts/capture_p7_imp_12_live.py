"""P7-IMP-12 live proof: current-year Temporal Activation on /result."""
...
def _engine_slice(payload: dict[str, object]) -> dict[str, object]:
    ...
    context = interpret_and_bind_temporal_activation(
        interpret_and_bind_luck_interaction(
            interpret_and_bind_luck_activation(
                interpret_and_bind_domain_interpretation(
                    interpret_and_bind_evidence_priority(
    ...
    for domain_id in ("authority", "career", "wealth", "relationship", "legacy", "vitality"):
        ...
        luck_item = activation.items[domain_id]
        annual_item = temporal.domain_results[domain_id]
        domains[domain_id] = {
            "natal_state": natal.state.value,
            "luck_activation": luck_item.activation_state.value,
            "annual_modifier": annual_item.annual_modifier,
            "annual_expression": annual_item.annual_expression_state,
            "temporal_driver": annual_item.temporal_driver,
            "temporal_bottleneck": annual_item.temporal_bottleneck,
            "stress": annual_item.stress,
            "recovery": annual_item.recovery,
            "conditions": list(annual_item.conditions),
        }
    return {
        "cycle_id": activation.cycle_id,
        "luck_window": activation.time_window,
        "annual_year": temporal.time_window,
        "annual_pillar": temporal.layer_results["annual"].temporal_pillar,
        "annual_source": temporal.layer_results["annual"].source_identity,
        "evaluated_layers": list(temporal.evaluated_layers),
        "shells": {
            layer: temporal.layer_results[layer].state.value
            for layer in ("monthly", "daily", "hourly")
        },
        "temporal_salience": list(temporal.temporal_salience),
        "dominant_activation": temporal.dominant_activation,
        "dominant_suppression": temporal.dominant_suppression,
        "domains": domains,
        "luck_activation": to_jsonable(activation),
        "luck_interaction": to_jsonable(interaction),
        "temporal_activation": to_jsonable(temporal),
        "natal_states": {
...
def main() -> None:
    """Restart runtime and capture Temporal Activation live proof."""
    ...
    try:
        ...
        annual = luck.get("annual") if isinstance(luck.get("annual"), dict) else {}
        ...
        overview = SHOTS / "p7_imp_12_result_overview.png"
        current = SHOTS / "p7_imp_12_luck_current.png"
        activation_shot = SHOTS / "p7_imp_12_luck_activation.png"
        interaction_shot = SHOTS / "p7_imp_12_luck_interaction.png"
        annual_shot = SHOTS / "p7_imp_12_annual_summary.png"
        compare_shot = SHOTS / "p7_imp_12_annual_comparison.png"
        expanded = SHOTS / "p7_imp_12_annual_domain_expanded.png"
        mobile = SHOTS / "p7_imp_12_mobile_annual.png"
        diag_shot = SHOTS / "p7_imp_12_diagnostics.png"
        with sync_playwright() as playwright:
            ...
            if "năm nay chắc chắn" in content.lower():
                raise RuntimeError("Ambiguous year copy leaked onto /result")
            ...
            page.locator("[data-luck-section='interaction']").scroll_into_view_if_needed()
            page.locator("[data-luck-section='interaction']").screenshot(path=str(interaction_shot))
            annual_el = page.locator("[data-luck-section='annual']")
            annual_el.scroll_into_view_if_needed()
            annual_el.screenshot(path=str(annual_shot))
            grid = page.locator("[data-luck-section='annual'] .bte-luck__annual-grid")
            grid.screenshot(path=str(compare_shot))
            first = page.locator("[data-luck-annual-id]").first
            first.locator(".bte-luck__annual-row-btn").click()
            ...
            mobile_page.locator("[data-luck-section='annual']").scroll_into_view_if_needed()
            mobile_page.locator("[data-luck-section='annual']").screenshot(path=str(mobile))
        ...
        if diag_body.get("temporal") != "PASS":
            raise RuntimeError(f"Temporal diagnostics not PASS: {diag_body.get('temporal')}")
        if not annual:
            raise RuntimeError("Customer luck annual summary missing")
        proof = {
            ...
            "analyze": {
                ...
                "annual_identity": luck.get("annual_identity"),
            ...
            "screenshots": {
                ...
                "annual_summary": str(annual_shot.relative_to(REPO)).replace("\\", "/"),
                "annual_comparison": str(compare_shot.relative_to(REPO)).replace("\\", "/"),
                "annual_domain_expanded": str(expanded.relative_to(REPO)).replace("\\", "/"),
                "mobile_annual": str(mobile.relative_to(REPO)).replace("\\", "/"),
                "diagnostics": str(diag_shot.relative_to(REPO)).replace("\\", "/"),
            },
        }
        (OUT / "P7-IMP-12_diagnostics.json").write_text(
