"""Report service coordination layer."""
...
class ReportService(ABC):
    """Coordinates report requests without business logic."""

    @abstractmethod
    def execute(self, request: AnalyzeRequest) -> ReportResponse:
        """Execute report generation through the adapter boundary."""


class DefaultReportService(ReportService):
    """Report service that delegates to ReportAdapter."""

    def __init__(self, adapter: ReportAdapter | None = None) -> None:
        self._adapter = adapter or ReportAdapter()

    def execute(self, request: AnalyzeRequest) -> ReportResponse:
        """Delegate report execution to the adapter."""
