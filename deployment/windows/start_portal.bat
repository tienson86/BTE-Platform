...
if exist ".venv\Scripts\python.exe" (
  ".venv\Scripts\python.exe" -m uvicorn applications.customer_portal.app:app --host %HOST% --port %PORT% >> "logs\portal.log" 2>&1
) else (
  python -m uvicorn applications.customer_portal.app:app --host %HOST% --port %PORT% >> "logs\portal.log" 2>&1
)
endlocal
