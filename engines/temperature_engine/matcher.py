"""Matcher for Temperature Engine rules."""
...
class TemperatureMatcher:
    """Evaluate rule conditions against TemperatureContext."""
    ...
    def is_active(self, rule: dict[str, Any]) -> bool:
        if str(rule.get("status") or "active") != "active":
            return False
        return str(rule.get("enabled") or "true").lower() not in {"false", "0", "no"}
