...
class RuleContextBuilder:
    ...
    def build(
        ...
        *,
        ...
        context["temperature"] = self._build_temperature(
            temperature,
            context["bazi"],
            context.get("wuxing"),
        )
        context["facts"] = self._build_facts(context)
        # Special case: publish FollowPatternCalculator label only (no second detector).
        follow_type = str((context.get("pattern") or {}).get("follow_type") or "").strip()
        if follow_type:
            context["special"] = {
                "case_name": follow_type,
                "source": "follow_type",
                "available": True,
            }
        else:
    ...
    def _build_pattern(self, pattern: Any) -> dict[str, Any]:
        ...
        return {
            ...
            "clash_status": _attr("clash_status"),
            "pattern_rank": _attr("pattern_rank"),
            "pattern_quality": _attr("pattern_quality"),
            "success_reason": _attr("success_reason"),
            "failure_reason": _attr("failure_reason"),
    ...
    def _useful_god_element(
        self,
        god_name: str,
        bazi_section: Mapping[str, Any],
    ) -> str | None:
        """Resolve English element of a ten-god name relative to day master."""
        ...
        # Inverse: which role → which element
        role_element = {
            "same": dm_el,
            "output": maps.GENERATES[dm_el],
            "wealth": maps.CONTROLS[dm_el],
            "resource": next(
                (src for src, dst in maps.GENERATES.items() if dst == dm_el),
                None,
            ),
            "officer": next(
                (src for src, dst in maps.CONTROLS.items() if dst == dm_el),
                None,
            ),
        }
        for role, (yang_name, yin_name) in maps.TEN_GOD_MATRIX[dm_el].items():
            if god_name in {yang_name, yin_name}:
                return role_element.get(role)
        return None

    @staticmethod
    def _mac_dinh_hy_ky(element: str) -> tuple[list[str], list[str]]:
        """
        Default Hỷ / Kỵ from useful-god element (HT001–HT005 / KT001–KT005).

        Hỷ = hành sinh Dụng thần; Kỵ = hành khắc Dụng thần.
        """
        element_vi = {
            "wood": "Mộc",
            "fire": "Hỏa",
        ...
        dung = element_vi.get(element, element)
        # GENERATES[src] == dung → src is Hỷ; CONTROLS[src] == dung → src is Kỵ
        generates_vi = {
            "Mộc": "Hỏa",
            "Hỏa": "Thổ",
            "Thổ": "Kim",
        ...
        hy = [src for src, dst in generates_vi.items() if dst == dung]
        ky = [src for src, dst in controls_vi.items() if dst == dung]
        return hy, ky
    ...
    def _build_temperature(
        ...
        temperature: Any,
        bazi_section: Mapping[str, Any] | None = None,
        wuxing: Mapping[str, Any] | None = None,
    ) -> dict[str, Any]:
        """
        Temperature namespace for Knowledge Base.

        When no upstream Temperature engine payload is supplied, derive
        status/result from month-branch cold/hot scores (TEMP001–012 / CLS).
        """
        stubs = {
            "status": None,
            "result": None,
        ...
        # Derive from month branch (KB cold_hot_rules TEMP001–012)
        bazi_section = bazi_section or {}
        ...
        if not month_branch or month_branch not in cold_hot:
            return stubs

        cold_score, hot_score = cold_hot[month_branch]
        status = self._classify_temperature(cold_score, hot_score)
        # Optional season bias hint from wuxing
        del wuxing  # reserved — month branch is primary producer
        data = dict(stubs)
        data["cold_score"] = cold_score
        data["hot_score"] = hot_score
        data["status"] = status
        data["result"] = status
        data["profile"] = status
        data["index"] = hot_score - cold_score
        return data

    @staticmethod
    def _classify_temperature(cold_score: int, hot_score: int) -> str:
        """Classify month cold/hot into CLS001–007 temperature_result codes."""
        bands = (
            (0, 2, 0, 2, "neutral"),
            (3, 5, 0, 2, "slightly_cold"),
            (6, 9, 0, 3, "cold"),
            (10, 99, 0, 4, "very_cold"),
            (0, 2, 3, 5, "slightly_hot"),
            (0, 3, 6, 9, "hot"),
            (0, 4, 10, 99, "very_hot"),
        )
        for c_min, c_max, h_min, h_max, label in bands:
            if c_min <= cold_score <= c_max and h_min <= hot_score <= h_max:
                return label
    ...
    def _build_facts(self, context: Mapping[str, Any]) -> dict[str, bool]:
        ...
        # Follow / tong — set from PatternResult.follow_type when present
        follow_type = str(pattern.get("follow_type") or "").strip()
        facts.setdefault("tong_vuong_confirmed", False)
        facts.setdefault("tong_nhuoc_confirmed", False)
        facts.setdefault("tong_tai_confirmed", False)
        facts.setdefault("tong_sat_confirmed", False)
        facts.setdefault("pattern_la_tong_cach", False)
        facts.setdefault("pattern_la_tong_vuong", False)
        facts.setdefault("pattern_la_tong_nhuoc", False)
        facts.setdefault("tong_cach_thuan", False)
        facts.setdefault("tong_cach_on_dinh", False)
        if follow_type:
            facts["pattern_la_tong_cach"] = True
            facts["tong_cach_thuan"] = True
            facts["tong_cach_on_dinh"] = True
            lower = follow_type.lower()
            if "vượng" in follow_type or "vuong" in lower:
                facts["tong_vuong_confirmed"] = True
                facts["pattern_la_tong_vuong"] = True
            if "nhược" in follow_type or "nhuoc" in lower:
                facts["tong_nhuoc_confirmed"] = True
                facts["pattern_la_tong_nhuoc"] = True
            if "tài" in follow_type or "tai" in lower:
                facts["tong_tai_confirmed"] = True
            if "sát" in follow_type or "sat" in lower:
                facts["tong_sat_confirmed"] = True

        # Follow / combination / temperature geometry — defaults stay False
        # until upstream engines provide additional signals.
        for key in (
            "hoa_khi_confirmed",
        ...
        # Temperature facts from derived temperature namespace
        temp_status = str(temperature.get("status") or temperature.get("result") or "")
        if temp_status:
            facts["temperature_profile_calculated"] = True
            facts["temperature_balanced"] = temp_status == "neutral"
            facts["temperature_excess_cold"] = temp_status in {
                "cold",
                "very_cold",
            }
            facts["temperature_excess_hot"] = temp_status in {
                "hot",
                "very_hot",
            }
            facts["chart_cold"] = temp_status in {
                "slightly_cold",
                "cold",
                "very_cold",
            }
            facts["chart_hot"] = temp_status in {
                "slightly_hot",
                "hot",
                "very_hot",
            }
