...
class InterpretationResult:
    ...
    # Sprint 4 — optional LuckContext (not serialized to portal by default)
    luck_context: Any | None = None

    def to_portal_dict(self) -> Dict[str, Any]:
        """Serialize InterpretationView for API / Portal (no internal fields)."""
        from .portal_view import build_portal_dict

        return build_portal_dict(self)
...
class InterpretationBuilder:
    ...
    def build(
        ...
        score = sum(
            float(getattr(section, "score", 0) or 0)
