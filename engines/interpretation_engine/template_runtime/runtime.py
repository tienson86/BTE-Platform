...
class TemplateRuntime(BaseRuntime):
    """Template Runtime facade implementing the Pack 03 runtime contract."""

    def __init__(
        self,
        *,
        runtime_id: str = "template_runtime",
        registry: TemplateRuntimeRegistry | None = None,
    ) -> None:
        """Initialize with optional injected registry."""
        super().__init__(runtime_id=runtime_id)
        self._registry = registry or TemplateRuntimeRegistry()

    @property
    def registry(self) -> TemplateRuntimeRegistry:
