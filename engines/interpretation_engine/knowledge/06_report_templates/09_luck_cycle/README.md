...
Module `09_luck_cycle` định nghĩa khung Template Framework cho phần **Đại Vận** trong báo cáo BTE.

Đây **không** phải nơi viết nội dung luận giải.
Đây **không** sinh dữ liệu luận giải.
Chỉ cung cấp cấu trúc để Report Engine render phần `luck_cycle`.
...
```text
Interpretation Engine  →  nội dung luận giải Đại Vận
Priority Engine        →  chọn kết quả ưu tiên
Report Engine          →  chọn template từ module này
09_luck_cycle/              →  cấu trúc sections / blocks / placeholders
Final Report           →  phần Đại Vận trong báo cáo
```
...
- `section_id`: semantic — ví dụ `luck_cycle_intro`, `luck_cycle_body`
- `block_id`: semantic — ví dụ `luck_cycle_B001`, `luck_cycle_B002`
