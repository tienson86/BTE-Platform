...
Module `07_children` định nghĩa khung Template Framework cho phần **Con Cái** trong báo cáo BTE.

Đây **không** phải nơi viết nội dung luận giải.
Đây **không** sinh dữ liệu luận giải.
Chỉ cung cấp cấu trúc để Report Engine render phần `children`.
...
```text
Interpretation Engine  →  nội dung luận giải Con Cái
Priority Engine        →  chọn kết quả ưu tiên
Report Engine          →  chọn template từ module này
07_children/              →  cấu trúc sections / blocks / placeholders
Final Report           →  phần Con Cái trong báo cáo
```
...
- `section_id`: semantic — ví dụ `children_intro`, `children_body`
- `block_id`: semantic — ví dụ `children_B001`, `children_B002`
