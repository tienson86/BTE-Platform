...
Module `02_personality` định nghĩa khung Template Framework cho phần **Tính Cách** trong báo cáo BTE.

Đây **không** phải nơi viết nội dung luận giải.
Đây **không** sinh dữ liệu luận giải.
Chỉ cung cấp cấu trúc để Report Engine render phần `personality`.
...
```text
Interpretation Engine  →  nội dung luận giải Tính Cách
Priority Engine        →  chọn kết quả ưu tiên
Report Engine          →  chọn template từ module này
02_personality/              →  cấu trúc sections / blocks / placeholders
Final Report           →  phần Tính Cách trong báo cáo
```
...
- `section_id`: semantic — ví dụ `personality_intro`, `personality_body`
- `block_id`: semantic — ví dụ `personality_B001`, `personality_B002`
