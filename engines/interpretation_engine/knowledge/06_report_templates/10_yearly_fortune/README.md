...
Module `10_yearly_fortune` định nghĩa khung Template Framework cho phần **Vận Hạn Năm** trong báo cáo BTE.

Đây **không** phải nơi viết nội dung luận giải.
Đây **không** sinh dữ liệu luận giải.
Chỉ cung cấp cấu trúc để Report Engine render phần `yearly_fortune`.
...
```text
Interpretation Engine  →  nội dung luận giải Vận Hạn Năm
Priority Engine        →  chọn kết quả ưu tiên
Report Engine          →  chọn template từ module này
10_yearly_fortune/              →  cấu trúc sections / blocks / placeholders
Final Report           →  phần Vận Hạn Năm trong báo cáo
```
...
```text
yearly_fortune_NNN.json

Ví dụ:
  yearly_fortune_001.json
  yearly_fortune_002.json
```
