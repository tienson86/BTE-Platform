...
Module `08_useful_god` định nghĩa khung Template Framework cho phần **Dụng Thần** trong báo cáo BTE.

Đây **không** phải nơi viết nội dung luận giải.
Đây **không** sinh dữ liệu luận giải.
Chỉ cung cấp cấu trúc để Report Engine render phần `useful_god`.
...
```text
Interpretation Engine  →  nội dung luận giải Dụng Thần
Priority Engine        →  chọn kết quả ưu tiên
Report Engine          →  chọn template từ module này
08_useful_god/              →  cấu trúc sections / blocks / placeholders
Final Report           →  phần Dụng Thần trong báo cáo
```
...
- `section_id`: semantic — ví dụ `useful_god_intro`, `useful_god_body`
- `block_id`: semantic — ví dụ `useful_god_B001`, `useful_god_B002`
