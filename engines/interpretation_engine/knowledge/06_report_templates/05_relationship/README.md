...
Module `05_relationship` định nghĩa khung Template Framework cho phần **Quan Hệ** trong báo cáo BTE.

Đây **không** phải nơi viết nội dung luận giải.
Đây **không** sinh dữ liệu luận giải.
Chỉ cung cấp cấu trúc để Report Engine render phần `relationship`.
...
```text
Interpretation Engine  →  nội dung luận giải Quan Hệ
Priority Engine        →  chọn kết quả ưu tiên
Report Engine          →  chọn template từ module này
05_relationship/              →  cấu trúc sections / blocks / placeholders
Final Report           →  phần Quan Hệ trong báo cáo
```
...
- `section_id`: semantic — ví dụ `relationship_intro`, `relationship_body`
- `block_id`: semantic — ví dụ `relationship_B001`, `relationship_B002`
...
```text
relationship_NNN.json

Ví dụ:
  relationship_001.json
  relationship_002.json
```
...
| Trường | Giá trị |
|--------|---------|
| module_id | 05_relationship |
| module_title | Quan Hệ |
| version | 1.0.0 |
