...
Module `03_career` định nghĩa khung Template Framework cho phần **Sự Nghiệp** trong báo cáo BTE.

Đây **không** phải nơi viết nội dung luận giải.
Đây **không** sinh dữ liệu luận giải.
Chỉ cung cấp cấu trúc để Report Engine render phần `career`.
...
```text
Interpretation Engine  →  nội dung luận giải Sự Nghiệp
Priority Engine        →  chọn kết quả ưu tiên
Report Engine          →  chọn template từ module này
03_career/              →  cấu trúc sections / blocks / placeholders
Final Report           →  phần Sự Nghiệp trong báo cáo
```
...
- `section_id`: semantic — ví dụ `career_intro`, `career_body`
- `block_id`: semantic — ví dụ `career_B001`, `career_B002`
