...
class SentenceRuntime(BaseRuntime):
    """Sentence Runtime facade implementing the Pack 03 runtime contract."""

    def __init__(
        self,
        *,
        runtime_id: str = "sentence_runtime",
        registry: SentenceRuntimeRegistry | None = None,
    ) -> None:
        """Initialize with optional injected registry."""
        super().__init__(runtime_id=runtime_id)
        self._registry = registry or SentenceRuntimeRegistry()

    @property
    def registry(self) -> SentenceRuntimeRegistry:
