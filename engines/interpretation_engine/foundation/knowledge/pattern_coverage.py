"""Coverage index and quality report for Pattern knowledge V1."""
...
from engines.interpretation_engine.foundation.knowledge.entity_types import PATTERN_KEYS
...
PATTERN_ORDER: tuple[str, ...] = PATTERN_KEYS


@dataclass(frozen=True, slots=True)
class PatternQualityReport:
    """Machine-readable Pattern knowledge quality report."""
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        return {
            ...
            "pattern_status": dict(self.pattern_status),
        }

    def to_markdown(self) -> str:
        """Render coverage index."""
        lines = ["# Pattern Knowledge V1", "", "## Patterns", ""]
        for key in PATTERN_ORDER:
            status = self.pattern_status.get(key, "MISSING")
...
def build_pattern_quality_report(
    *,
    ...
    pattern_status = {
        key: _status(by_key.get(key)) for key in PATTERN_ORDER
    }
...
def write_pattern_reports(
    *,
    knowledge_root: Path | None = None,
    report: PatternQualityReport | None = None,
) -> tuple[Path, Path]:
    """Write coverage markdown and machine-readable quality JSON."""
    payload = report or build_pattern_quality_report()
    root = knowledge_root or DEFAULT_KNOWLEDGE_ROOT
    coverage_path = root / "domains" / "pattern" / "COVERAGE.md"
    quality_path = root / "reports" / "pattern_k4_quality.json"
...
def _status(entity: KnowledgeEntity | None) -> str:
    """Return coverage status label for one pattern."""
    if entity is None:
        return "MISSING"
