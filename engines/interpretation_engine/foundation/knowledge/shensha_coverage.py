"""Coverage index and quality report for Shen Sha Expert Ready knowledge."""
...
from engines.interpretation_engine.foundation.knowledge.entity_types import SHEN_SHA_KEYS
...
SHEN_SHA_ORDER: tuple[str, ...] = SHEN_SHA_KEYS


@dataclass(frozen=True, slots=True)
class ShenShaQualityReport:
    """Machine-readable Shen Sha knowledge quality report."""

    entity_count: int
    approved_count: int
    draft_count: int
    expert_ready_count: int
    concept_count: int
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        return {
            ...
            "expert_ready_count": self.expert_ready_count,
            ...
            "shensha_status": dict(self.shensha_status),
        }

    def to_markdown(self) -> str:
        """Render coverage index."""
        lines = ["# Shen Sha Knowledge V1 — Expert Ready", "", "## Shen Sha", ""]
        for key in SHEN_SHA_ORDER:
            status = self.shensha_status.get(key, "MISSING")
            lines.append(f"{key:<24} {status}")
        lines.extend(
            [
                "",
                "## Coverage",
                ...
                f"- expert ready: {self.expert_ready_count}",
...
def build_shensha_quality_report(
    *,
    ...
    broken = tuple(
        ...
        if issue.code in {"broken_reference", "broken_concept_reference", "broken_concepts"}
    )
    gate = KnowledgeQualityGate()
    quality = gate.evaluate_approved(entities)
    expert = gate.evaluate_expert_ready(entities)
    missing_required = tuple(
        f"{issue.entity_id}: {issue.code}" for issue in (*quality.issues, *expert.issues)
    )
    duplicates = DuplicateContentDetector().detect(entities)
    application_counts: Counter[str] = Counter()
    recommendation_entities = 0
    warning_entities = 0
    expert_ready_count = 0
    for entity in entities:
        ...
        if any(
            ...
            warning_entities += 1
        if (
            entity.metadata.status.value == "approved"
            and not gate.evaluate(entity).issues
        ):
            expert_ready_count += 1
    by_key = {entity.key: entity for entity in entities}
    shensha_status = {key: _status(by_key.get(key)) for key in SHEN_SHA_ORDER}
    return ShenShaQualityReport(
        ...
        expert_ready_count=expert_ready_count,
        concept_count=len(
            {concept_id for entity in entities for concept_id in entity.concept_ids}
        ),
        broken_references=broken,
        missing_required_content=tuple(dict.fromkeys(missing_required)),
        ...
        shensha_status=shensha_status,
    )


def write_shensha_reports(
    *,
    knowledge_root: Path | None = None,
    report: ShenShaQualityReport | None = None,
) -> tuple[Path, Path]:
    """Write coverage markdown and machine-readable quality JSON."""
    payload = report or build_shensha_quality_report()
    root = knowledge_root or DEFAULT_KNOWLEDGE_ROOT
    coverage_path = root / "domains" / "shensha" / "COVERAGE.md"
    quality_path = root / "reports" / "shensha_k6_quality.json"
...
def _status(entity: KnowledgeEntity | None) -> str:
    """Return coverage status label for one Shen Sha."""
    if entity is None:
        return "MISSING"
    if entity.metadata.status.value == "approved":
        return "EXPERT_READY"
