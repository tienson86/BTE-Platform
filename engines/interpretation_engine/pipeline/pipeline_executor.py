"""Interpretation pipeline executor for multi-stage orchestration."""
...
class PipelineExecutor:
    """Orchestrates ordered interpretation stage execution without business logic."""
        ...
        for _attempt in range(1, attempts + 1):
            try:
                ...
                last_outcome = StageOutcome(
                    ...
                    payload={},
                    messages=(
                        f"stage_orchestration_error:{type(exc).__name__}:{exc}",
                    ),
                )
    ...
    def _validate_required_stages(
        ...
        if missing:
            raise InterpretationPipelineError(
                f"missing_required_stages:{','.join(missing)}"
            )
