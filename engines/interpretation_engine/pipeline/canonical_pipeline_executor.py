"""Deterministic Canonical Interpretation Pipeline executor."""
...
from engines.interpretation_engine.pipeline.interpretation_trace import InterpretationPipelineTraceStep
from engines.interpretation_engine.pipeline.package_contract import InterpretationPackageContractVerifier
...
StageHandler = Callable[["InterpretationPipelineContext"], Mapping[str, Any]]
...
@dataclass
class InterpretationPipelineContext:
    """Append-only interpretation pipeline context. Upstream outputs are immutable."""

    analysis_input: Any = None
    decision_input: Any = None
    luck_input: Any = None
    foundation_input: Any = None
    knowledge_input: Any = None
    diagnostics: list[InterpretationPipelineDiagnostic] = field(default_factory=list)
    ...
    def add_diagnostic(self, item: InterpretationPipelineDiagnostic) -> None:
    ...
    def publish(
        ...
        *,
        ...
        if stage_id in self._stage_payloads:
            raise InterpretationDuplicatePublicationError(f"duplicate_execution:{stage_id}")
        snapshot = dict(payload)
        for name in declared_outputs:
            if name in self._field_owners:
                raise InterpretationDuplicatePublicationError(
    ...
    def foundation_result(self) -> dict[str, Any] | None:
        """Published IE-1 foundation snapshot."""
        value = self.get_output("foundation_result")
        return value if isinstance(value, dict) else None

    @property
    def knowledge_result(self) -> dict[str, Any] | None:
        """Published IE-2 knowledge selection snapshot."""
        value = self.get_output("knowledge_result")
        return value if isinstance(value, dict) else None

    @property
    def composition_result(self) -> dict[str, Any] | None:
        """Published IE-3 composition snapshot."""
        value = self.get_output("composition_result")
        return value if isinstance(value, dict) else None


class InterpretationPipelineExecutor:
    """Execute enabled interpretation stages once, in dependency order only."""
    ...
    def execute(
        self,
        *,
        registry: InterpretationStageRegistry,
        stage_order: Sequence[str],
        context: InterpretationPipelineContext,
        handlers: Mapping[str, StageHandler],
    ) -> tuple[InterpretationPipelineTraceStep, ...]:
        """Execute each enabled stage once. Disabled stages are skipped."""
        steps: list[InterpretationPipelineTraceStep] = []
        for stage_id in stage_order:
            ...
            if handler is None:
                raise CanonicalInterpretationPipelineError(f"missing_stage_handler:{stage_id}")
    ...
    def _verify_stage(
        self,
        record: InterpretationStageRecord,
        context: InterpretationPipelineContext,
    ) -> None:
        ...
        if record.stage_id == STAGE_FOUNDATION:
            self._verifier.verify_foundation_component(interpretation_version="1.0.0")
            if not isinstance(context.analysis_input, Mapping):
                return
            self._verifier.verify_knowledge_component(
            ...
            return
        if record.stage_id == STAGE_KNOWLEDGE:
            if not isinstance(context.analysis_input, Mapping):
                return
            self._verifier.verify_knowledge_component(
                knowledge_version="1.0.0",
            ...
            return
        if record.stage_id == STAGE_COMPOSITION:
            if not isinstance(context.analysis_input, Mapping):
                return
            self._verifier.verify_composition_component(
