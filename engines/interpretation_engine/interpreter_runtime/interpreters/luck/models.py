"""Luck Interpreter result models (Pack 03 contracts)."""
...
class LuckItemResult:
    """One interpreted Dayun / Liunian / Liuyue / Interaction item."""
...
@dataclass(frozen=True, slots=True)
class LuckComponentResult:
    """One interpreted Luck domain component."""

    component_id: str
    value: str
    score: float = 0.0
    count: int = 0
    items: tuple[LuckItemResult, ...] = ()
    description: str = ""
    attributes: Mapping[str, Any] = field(default_factory=dict)
...
@dataclass(frozen=True, slots=True)
class LuckInterpretationSection:
    """Typed Luck Interpreter output."""
    ...
    luck_score: float
    components: Mapping[str, LuckComponentResult] = field(default_factory=dict)
    ...
    def validate(self) -> bool:
        ...
        if not self.section.validate():
            return False
        if self.section.section_type != "luck":
            return False
    ...
    def to_attributes(self) -> dict[str, Any]:
        ...
        return {
            "dayun": [item.to_dict() for item in self.dayun],
            "liunian": [item.to_dict() for item in self.liunian],
            "liuyue": [item.to_dict() for item in self.liuyue],
