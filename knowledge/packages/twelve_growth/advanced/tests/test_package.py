"""Package-level tests for bz_14_twelve_growth_advanced KX-6B. No engine imports."""
...
ALLOWED = {
    ...
    "ten_gods_profile", "ten_gods_balance", "ten_gods_dominance", "resolved_useful_god",
    "decision_priority", "resolution_confidence",
}
OUTPUTS = {
    "growth_phase", "growth_profile", "growth_balance", "growth_score",
    "growth_confidence", "growth_reasoning", "growth_diagnostics",
}
FORBIDDEN = {
    ...
    "follow_pattern_type", "transformation_detected", "transformation_type", "ten_gods_score",
}
...
def test_package_identity() -> None:
    ...
    assert package["package_id"] == "bz_14_twelve_growth_advanced"
    ...
    assert package["domain_id"] == "DOM-TWELVE_GROWTH"
...
def test_contracts() -> None:
    ...
    assert "growth_narrative" not in outputs


def test_duplicate_ids() -> None:
    ids = [item["id"] for item in _rules()]
    assert 350 <= len(ids) <= 370
    assert len(ids) == len(set(ids))
    assert sorted(ids)[0] == "TGP-000001"
    assert sorted(ids)[-1] == "TGP-000360"
...
def test_reasoning_completeness() -> None:
    ...
    slugs = ("prosperous", "declining", "balanced", "dominant", "transition", "border", "lowconf", "mixed")
    for slug in slugs:
        ...
        assert any(r.startswith("TGP-") for r in chain["rule_ids"])
...
def test_serialization_round_trip() -> None:
    ...
    assert json.loads(encoded)["package_id"] == "bz_14_twelve_growth_advanced"
...
def test_upstream_ids_exist() -> None:
    ...
    for slug in ("prosperous", "declining", "balanced", "dominant", "transition", "border", "lowconf", "mixed"):
        chain = _load(f"reasoning/chains/{slug}.json")
