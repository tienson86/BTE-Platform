"""Package-level tests for bz_15_hidden_stems_advanced KX-6C. No engine imports."""
...
ALLOWED = {
    ...
    "follow_pattern_score", "transformation_score", "interaction_score", "interaction_confidence",
    "ten_gods_profile", "ten_gods_balance", "growth_profile", "growth_score",
    "resolved_useful_god", "decision_priority", "resolution_confidence",
}
OUTPUTS = {
    "hidden_stems_profile", "hidden_stems_balance", "hidden_stems_dominance",
    "hidden_stems_score", "hidden_stems_confidence", "hidden_stems_reasoning", "hidden_stems_diagnostics",
}
FORBIDDEN = {
    ...
    "hidden_stem", "ten_gods_dominance", "growth_phase", "transformation_detected",
}
...
def test_package_identity() -> None:
    ...
    assert package["package_id"] == "bz_15_hidden_stems_advanced"
    ...
    assert package["domain_id"] == "DOM-HIDDEN_STEM"
...
def test_contracts() -> None:
    ...
    assert "hidden_stems_narrative" not in outputs


def test_duplicate_ids() -> None:
    ids = [item["id"] for item in _rules()]
    assert 370 <= len(ids) <= 390
    assert len(ids) == len(set(ids))
    assert sorted(ids)[0] == "HSA-000001"
    assert sorted(ids)[-1] == "HSA-000380"
...
def test_reasoning_completeness() -> None:
    ...
    slugs = ("strong_roots", "weak_roots", "balanced", "dominant", "special", "border", "lowconf", "mixed")
    for slug in slugs:
        ...
        assert chain["upstream_ten_gods_rules"]
        assert chain["upstream_twelve_growth_rules"]
        assert any(r.startswith("HSA-") for r in chain["rule_ids"])
...
def test_serialization_round_trip() -> None:
    ...
    assert json.loads(encoded)["package_id"] == "bz_15_hidden_stems_advanced"
...
def test_upstream_ids_exist() -> None:
    ...
    maps = {
        ...
        "upstream_ten_gods_rules": collect("ten_gods/advanced"),
        "upstream_twelve_growth_rules": collect("twelve_growth/advanced"),
    }
    for slug in ("strong_roots", "weak_roots", "balanced", "dominant", "special", "border", "lowconf", "mixed"):
        chain = _load(f"reasoning/chains/{slug}.json")
