...
The Report Registry catalogs metadata for Report Templates (`RPT-*`) owned by Report Templates.

This directory stores **metadata catalog** artifacts only. Authoritative content remains in `knowledge/report_templates/`.

---

## 2. Purpose

- Register metadata for `RPT-*` objects
...
| Layer | Role |
|-------|------|
| Source module (`knowledge/report_templates/`) | Authoritative object content |
| This registry (`knowledge/registry/report_registry/`) | Metadata catalog / locator |
...
| File | Role |
|------|------|
| `REPORT_REGISTRY_SPEC.md` | Domain specification |
| `report_registry.json` | Primary catalog |
...
Every record in `report_registry.json` SHALL conform to:
