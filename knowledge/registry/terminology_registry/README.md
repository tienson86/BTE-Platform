...
The Terminology Registry catalogs metadata for Terminology (`TERM-*`) owned by the Terminology Framework.

This directory stores **metadata catalog** artifacts only. Authoritative content remains in `knowledge/terminology/`.

---

## 2. Purpose

- Register metadata for `TERM-*` objects
...
| Layer | Role |
|-------|------|
| Source module (`knowledge/terminology/`) | Authoritative object content |
| This registry (`knowledge/registry/terminology_registry/`) | Metadata catalog / locator |
...
| File | Role |
|------|------|
| `TERMINOLOGY_REGISTRY_SPEC.md` | Domain specification |
| `terminology_registry.json` | Primary catalog |
...
Every record in `terminology_registry.json` SHALL conform to:
