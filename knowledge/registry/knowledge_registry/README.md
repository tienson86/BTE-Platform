...
The Knowledge Registry catalogs metadata for Knowledge Assets (`KNO-*`) owned by Knowledge Canon.

This directory stores **metadata catalog** artifacts only. Authoritative content remains in `knowledge/knowledge_canon/`.

---

## 2. Purpose

- Register metadata for `KNO-*` objects
...
| Layer | Role |
|-------|------|
| Source module (`knowledge/knowledge_canon/`) | Authoritative object content |
| This registry (`knowledge/registry/knowledge_registry/`) | Metadata catalog / locator |
...
| File | Role |
|------|------|
| `KNOWLEDGE_REGISTRY_SPEC.md` | Domain specification |
| `knowledge_registry.json` | Primary catalog |
| `domain_index.json` | Index file |
| `category_index.json` | Index file |
| `dependency_index.json` | Index file |
...
Every record in `knowledge_registry.json` SHALL conform to:
