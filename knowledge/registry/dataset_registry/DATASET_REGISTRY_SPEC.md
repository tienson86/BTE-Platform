...
> **Document ID:** REG-DATA-SPEC-001
>
> **Module:** `knowledge/registry/dataset_registry`
>
> **Version:** V1.0.0
>
> **Status:** Official
...
This specification defines the `dataset_registry` domain within the BTE Registry.

It catalogs metadata for objects identified by `CASE-NNNNNN`.
...
- Provide a stable metadata catalog for `CASE-*` objects
...
- Registry metadata records (`DREG-*`)
- Domain indexes
- Schema conformance
...
- Authoritative content in `knowledge/golden_dataset/`
...
| Field | Value |
|-------|-------|
| Registry Prefix | `DREG` |
| Object ID Prefix | `CASE` |
| Sequence Format | 6 digits |
| Example Registry ID | `DREG-000001` |
| Example Object ID | `CASE-000001` |
...
- TODO: Root Registry specs do not enumerate dedicated Dataset index files. Minimal classification indexes are provided pending Chief Architect confirmation.
...
This registry registers objects from `knowledge/golden_dataset/` and may declare dependencies on other Registry Records.
...
Every `CASE-*` object SHALL be registered before becoming available to production systems.
