...
The Dataset Registry catalogs metadata for Golden Dataset cases (`CASE-*`).

This directory stores **metadata catalog** artifacts only. Authoritative content remains in `knowledge/golden_dataset/`.

---

## 2. Purpose

- Register metadata for `CASE-*` objects
...
| Layer | Role |
|-------|------|
| Source module (`knowledge/golden_dataset/`) | Authoritative object content |
| This registry (`knowledge/registry/dataset_registry/`) | Metadata catalog / locator |
...
| File | Role |
|------|------|
| `DATASET_REGISTRY_SPEC.md` | Domain specification |
| `dataset_registry.json` | Primary catalog |
...
Every record in `dataset_registry.json` SHALL conform to:
